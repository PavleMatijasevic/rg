...
// camera
Camera camera(glm::vec3(25.0f, 5.0f, 0.0f));
...
int main() {
    ...
    Model stairs_model(FileSystem::getPath("resources/objects/stairs/stairs.obj").c_str());
    Model sofa_model(FileSystem::getPath("resources/objects/sofa/Sofa.obj").c_str());
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // stairs
        function.load_sofa(sofa_model, model, shader);
        model = glm::mat4(1.0f);
        function.load_stairs(stairs_model, model, shader);

        // floor
        glBindVertexArray(floorVAO);
        glBindTexture(GL_TEXTURE_2D, floor);
        function.setting_up_floor(shader, model, floor);

        // wall
        glBindVertexArray(cubeVAO);
        glBindTexture(GL_TEXTURE_2D, wall);
        function.setting_up_wall(shader, model, tile, wall, 0.5f);
        function.setting_up_pillar(shader, model, wall);
        function.setting_up_wall(shader, model, tile, wall, 6.5f);

        // tiles
        glBindTexture(GL_TEXTURE_2D, tile);
        function.setting_up_tiles_in_pillar(shader, model);
        function.setting_up_tiles_in_wall(shader, model);
        ...
        glBindVertexArray(lightVAO);
        function.setting_up_light(lightShader, model);
